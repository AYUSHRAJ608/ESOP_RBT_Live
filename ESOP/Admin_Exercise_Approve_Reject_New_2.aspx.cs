...
namespace ESOP
{
    public partial class Admin_Exercise_Approve_Reject_New : System.Web.UI.Page
    {
        ...
        {
            //GridView GrvPendingforApproval = e.Row.FindControl("GrvPendingforApproval") as GridView;              //Commented by Krutika on 06-02-23
            Repeater rptPendingforApproval = e.Row.FindControl("rptPendingforApproval") as Repeater;                //Added by Krutika on 06-02-23
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                for (int i = 0; i <= grdMain.Rows.Count; i++)
                {
                    ...
                    rptPendingforApproval.DataSource = ds.Tables[0];
                    rptPendingforApproval.DataBind();
                    //End
                }
                ViewState["PendingExercise"] = ds.Tables[0];
                //Added by Bhushan on 02-02-2023
            }
        }
        ...
        protected void GrvPendingforApproval_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            //try
            {
                ...
                if (e.CommandName == "Approve")
                {
                    ...
                    {
                        showmsg.Attributes["style"] = "color:green; font-weight:bold;text-align: center;";
                        showmsg.InnerText = "Record Accept successfully!.";
                        GET_Employee_Secretarial_Approve_Reject_Data();
                        BindMainGrid();
                    }
                }
        ...
        protected void rptPendingforApproval_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item) 
            {
                TextBox txtBox = (TextBox)e.Item.FindControl("txt_PerqAmt");
                if(txtBox != null)
                {
                    txtBox.TextChanged += new EventHandler(txt_PerqAmt_TextChanged);
                    if(txtBox.Text != "")
                    {
                        txt_PerqAmt_TextChanged(sender,e);
                    }
                }
            }
        }
