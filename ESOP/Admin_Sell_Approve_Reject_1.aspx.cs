...
namespace ESOP
{
    public partial class Admin_Sell_Approve_Reject_1 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                GET_EMPLOYEE_SELL_DATA1();

            }
        }

        protected void GET_EMPLOYEE_SELL_DATA1()
        {
            try
            {
                DataSet ds = new DataSet();
                ds = objBAL.GET_EMPLOYEE_SELL_DATA1(objBO);
                if (ds.Tables[0].Rows.Count > 0 || ds.Tables[1].Rows.Count > 0 || ds.Tables[2].Rows.Count > 0 || ds.Tables[3].Rows.Count > 0)
                {
                    ...
                    {
                        grdMain.DataSource = ds.Tables[3];
                        grdMain.DataBind();
                        btn_BulkApprove.Visible = true;
                        btn_BulkReject.Visible = true;
                    }
                    ...
                    if (checkbox.Checked)
                    {
                        ...
                        foreach (GridViewRow gvrow in GrvPendingforApproval.Rows)
                        {
                            if (gvrow.RowType == DataControlRowType.DataRow)
                            {
                                ...
                                objBO.CLIENT_ID = SID;
                                objBO.DIS_STATUS = "Approved";
                                //bool val = objBAL.update_status(objBO); // Commented by Nagesh
                                bool val = objBAL.update_status2(objBO);
        ...
        {
            DataSet ds = objBAL.GET_EMPLOYEE_SELL_DATA1(objBO);
        ...
        //Commented by Krutika on 23-02-23
        ...
        protected void grdMain_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                for (int i = 0; i <= grdMain.Rows.Count; i++)
                {
                    ...
                    ds = objBAL.GET_EMPLOYEE_SELL_DETAILS_DATA1(id);
        ...
        protected void grdMain_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                if (e.CommandName == "Download")
                {
                    ...
                    {
                        zip.AlternateEncodingUsage = ZipOption.AsNecessary;
                        zip.AddDirectoryByName(ECODE + "Sales_Docs_All");
                        ...
                        for (int i = 2; i <= 4; i++)
                        {
                            ...
                            zip.AddFile(filePath, ECODE + "Sales_Docs_All");
                        }
                        Response.Clear();
                ...
                if (e.CommandName == "Approve")
                {
                    ...
                    foreach (GridViewRow gvrow in GrvPendingforApproval.Rows)
                    {
                        if (gvrow.RowType == DataControlRowType.DataRow)
                        {
                            ...
                            //Created by Nagesh update_status2
                            bool val1 = objBAL.update_status2(objBO);
