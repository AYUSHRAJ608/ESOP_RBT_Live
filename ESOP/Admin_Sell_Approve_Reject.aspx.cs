...
namespace ESOP
{
    public partial class Admin_Sell_Approve_Reject : System.Web.UI.Page
    {
        ...
        protected void GrvPFADB(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                for (int i = 0; i <= grdMain.Rows.Count; i++)
                {
                    ...
                    ds = objBAL.GET_EMPLOYEE_SELL_DETAILS_DATA(id);
        ...
        protected void btn_bulkApprove_Click(object sender, EventArgs e)
        {
            try
            {
                    ...
                    if (checkbox.Checked)
                    {
                        ...
                        foreach (GridViewRow gvrow in GrvPendingforApproval.Rows)
                        {
                            if (gvrow.RowType == DataControlRowType.DataRow)
                            {
                                ...
                                //bool val = objBAL.update_status(objBO); //commented by Nagesh
                                bool val = objBAL.update_status1(objBO);
                ...
                showmsg.InnerText = "Sale allotment has been approved.";
            }
            catch (Exception ex)
                    ...
                    if (checkbox.Checked)
                    {
                        ...
                        foreach (GridViewRow gvrow in GrvPendingforApproval.Rows)
                        {
                            if (gvrow.RowType == DataControlRowType.DataRow)
                            {
                                ...
                                objBO.CLIENT_ID = SID;
                                objBO.APPROVE_STATUS = "Rejected";
                                //bool val = objBAL.update_status(objBO);
                                bool val = objBAL.update_status1(objBO); //commented by Nagesh
                                if (val == true)
                                {
            ...
            try
            {
                if (e.CommandName == "Download")
                {
                    ...
                    using (ZipFile zip = new ZipFile())
                    {
                        ...
                        zip.AddDirectoryByName(ECODE + "Sales_Doc");

                        objBO.ECODE = Convert.ToString(ECODE);

                        DataSet ds = objBAL.GET_EMP_SALE_DOC(objBO);
                        if (ds.Tables[0].Rows.Count > 0)
                        {
                            foreach (DataColumn dc in ds.Tables[0].Columns)
                            //for (int i = 2; i <= 4; i++)
                            {
                                int columnindex = ds.Tables[0].Rows[0].Table.Columns[dc.ColumnName].Ordinal;
                                if (columnindex > 1 && Convert.ToString(ds.Tables[0].Rows[0][columnindex]) != "")
                                {
                                    string filePath = Convert.ToString(ds.Tables[0].Rows[0][columnindex]);//ds.Tables[0].Rows[0][i].ToString();
                                    filePath = Server.MapPath(Convert.ToString(ds.Tables[0].Rows[0][columnindex]));//Server.MapPath(ds.Tables[0].Rows[0][i].ToString());
                                    zip.AddFile(filePath, ECODE + "Sales_Doc");
                                }
                            }
                        }
                        Response.Clear();
                ...
                if (e.CommandName == "Approve")
                {
                    ...
                    foreach (GridViewRow gvrow in GrvPendingforApproval.Rows)
                    {
                        if (gvrow.RowType == DataControlRowType.DataRow)
                        {
                            ...
                            //Created by Nagesh update_status1
                            bool val1 = objBAL.update_status1(objBO);
        ...
        protected void GrvPendingforApproval_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                else if (e.CommandName != "Page" && e.CommandName != "Sort")
                {
                    ...
                    Label No_of_Sale = (Label)gv1.Rows[Convert.ToInt32(rowIndex)].FindControl("lblOptions");
                    string VESTING_DETAIL_ID = Convert.ToString(gv1.DataKeys[rowIndex].Values[1]);
                    string SID = Convert.ToString(gv1.DataKeys[rowIndex].Values[2]);
