...
namespace ESOP
{
    public partial class admin_dashboard : System.Web.UI.Page
    {
        ...
        private void FIVE_YEAR_Laps()
        {
            DataSet ds = VestingBAL.FIVE_YEAR_Laps();
        }
        ...
        private DataTable CalculateTotal(DataTable dt)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    dr1["VID"] = Convert.ToString(dt.Rows[i]["vid"]);
                    dr1["V_DETAIL_ID"] = dt.Rows[i]["v_detail_id"].ToString();
                    dr1["VPERCENTAGE"] = dt.Rows[i]["vpercentage"].ToString();
                    dr1["GRANT_NAME"] = dt.Rows[i]["grant_name"].ToString();
                    dr1["GRANTED"] = Convert.ToDouble(dt.Rows[i]["granted"]);
                    dr1["VESTED"] = Convert.ToDouble(dt.Rows[i]["vested"]);
                    dr1["VESTED_PENDING"] = Convert.ToDouble(dt.Rows[i]["vested_pending"]);
                    dr1["EXERCISED"] = Convert.ToDouble(dt.Rows[i]["exercised"]);
                    dr1["EXERCISED_PENDING"] = Convert.ToDouble(dt.Rows[i]["exercised_pending"]);
                    dr1["SALE"] = Convert.ToDouble(dt.Rows[i]["sale"]);
                    dr1["LBV"] = Convert.ToDouble(dt.Rows[i]["lbv"]);
                    dr1["LAV"] = Convert.ToDouble(dt.Rows[i]["lav"]);
                    //added EXERCISE_DATE & SALE_DATE by Pallavi
                    dr1["EXERCISE_DATE"] = Convert.ToString(dt.Rows[i]["exercise_date"]);
                    dr1["SALE_DATE"] = Convert.ToString(dt.Rows[i]["sale_date"]);
                    dr1["TOTAL_LAPSE"] = Convert.ToDouble(dt.Rows[i]["total_lapse"]);
                    dr1["STOCK_IN_HAND"] = Convert.ToDouble(dt.Rows[i]["stock_in_hand"]);
                    dr1["VESTING_DATE"] = dt.Rows[i]["vesting_date"].ToString();//Convert.ToDateTime(dt.Rows[i][14]).ToString("MM/dd/yyyy");
                    dr1["LAPSE_DATE"] = dt.Rows[i]["lapse_date"].ToString();          //Added by Krutika on 19-05-22 for Showing lapse date on employee dashboard

                    newdt.Rows.Add(dr1);

                    //GRANTED += dt.Rows[i].Field<double>(4);
                    //VESTED += dt.Rows[i].Field<double>(5);
                    //VESTED_PENDING += dt.Rows[i].Field<double>(6);
                    //EXERCISED += dt.Rows[i].Field<double>(7);
                    //EXERCISED_PENDING += dt.Rows[i].Field<double>(8);
                    //SALE += dt.Rows[i].Field<double>(9);
                    //LBV += dt.Rows[i].Field<double>(10);
                    //LAV += dt.Rows[i].Field<double>(11);
                    //TOTAL_LAPSE += dt.Rows[i].Field<double>(12);
                    //STOCK_IN_HAND += dt.Rows[i].Field<double>(13);

                    GRANTED += Convert.ToDouble(dt.Rows[i]["granted"]);
                    VESTED += Convert.ToDouble(dt.Rows[i]["vested"]);//he index tak ok
                    VESTED_PENDING += Convert.ToDouble(dt.Rows[i]["vested_pending"]);
                    EXERCISED += Convert.ToDouble(dt.Rows[i]["exercised"]);
                    EXERCISED_PENDING += Convert.ToDouble(dt.Rows[i]["exercised_pending"]);
                    SALE += Convert.ToDouble(dt.Rows[i]["sale"]);
                    LBV += Convert.ToDouble(dt.Rows[i]["lbv"]);
                    LAV += Convert.ToDouble(dt.Rows[i]["lav"]);
                    TOTAL_LAPSE += Convert.ToDouble(dt.Rows[i]["total_lapse"]);
                    STOCK_IN_HAND += Convert.ToDouble(dt.Rows[i]["stock_in_hand"]);
