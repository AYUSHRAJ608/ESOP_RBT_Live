...
namespace ESOP
{
    public partial class LapseList : System.Web.UI.Page
    {
        ...
        private void BindMainGrid()
        {
            ...
            ds = PresidentBAL.GET_LAPSE_LIST(PresidentBO);
        ...
        protected void gvParent_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            //        gvSubChild.DataBind();
            //        //ScriptManager.RegisterStartupScript(this, this.GetType(), "Pop", "bindPicker();", true);
            //    }
            //    else
            //    {
        ...
        protected void TxtLAV_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                UpdatePanel updLAV = (UpdatePanel)row.FindControl("updLAV");
        ...
        protected void TxtLBV_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                UpdatePanel updLBV = (UpdatePanel)row.FindControl("updLBV");
                ...
                if ((Convert.ToString(TxtLBV.Text) == "" ? 0 : (Convert.ToDouble(TxtLBV.Text))) > Convert.ToInt32((VESTED_PENDING_1.Text)))
                {
                    ...
                    int x = 0;
                    TxtLBV.Text = Convert.ToString(x);
                    //upd.Update();
                    updLBV.Update();
        ...
        protected void BtnGrvApprove_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Chk.Checked)
                {
                    TextBox txt = (TextBox)row.FindControl("TxtLBV");
                    string LBV = txt.Text.Replace(",", "");

                    TextBox txt1 = (TextBox)row.FindControl("TxtLAV");
                    string LAV = txt1.Text.Replace(",", "");

                    TextBox txt2 = (TextBox)row.FindControl("TxtRemark");
                    string Remark = txt2.Text.Replace(",", "");

                    //TextBox txt3 = (TextBox)row.FindControl("txtDateOfLapse");
                        ...
                        Convert.ToString(Remark) != "")
                    {
                        ...
                        PresidentBO.Action = "APPROVED_BY_ADMIN";
                        ...
                        //PresidentBO.LapseDate = Convert.ToString(LapseDate).Trim();
        ...
        protected void BtnGrvReject_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Chk.Checked)
                {
                    ...
                    if (/*(Convert.ToString(LBV) == "" ? 0 : Convert.ToInt32(LBV)) != 0 && (Convert.ToString(LAV) == "" ? 0 : Convert.ToInt32(LAV)) != 0 && Convert.ToString(Remark) != "" &&*/
                        Convert.ToString(LapseDate) != "")
                    {
                        HiddenField HdID = (HiddenField)row.FindControl("Hd_Id");
                        HdID.Value = HdID.Value.ToString().Replace(",", "");
                        ...
                        PresidentBO.Action = "REJECTED_BY_ADMIN";
                        PresidentBO.GRANT_ID = HdGrantID.Value.ToString().Trim();
                        PresidentBO.V_ID = HdID.Value.ToString().Trim();
                        PresidentBO.VestingD_ID = hdVestingID.Value.ToString().Trim();
                        PresidentBO.LBV = Convert.ToString(LBV) == "" ? 0 : Convert.ToInt32(LBV);
            ...
            try
            {
                foreach (GridViewRow gvrow in gvParent.Rows)
                {
                    if (gvrow.RowType == DataControlRowType.DataRow)
                    {
                        ...
                        if (checkbox.Checked)
                        {
                            TextBox txt = (TextBox)gvrow.FindControl("TxtLBV");
                            string LBV = txt.Text.Replace(",", "");

                            TextBox txt1 = (TextBox)gvrow.FindControl("TxtLAV");
                            string LAV = txt1.Text.Replace(",", "");
                ...
                foreach (GridViewRow gvrow in gvParent.Rows)
                {
                    if (gvrow.RowType == DataControlRowType.DataRow)
                    {
                        ...
                        if (checkbox.Checked)
                        {
                            ...
                            TextBox txt3 = (TextBox)gvrow.FindControl("txtDateOfLapse");
                            string LapseDate = txt3.Text.Replace(",", "");
